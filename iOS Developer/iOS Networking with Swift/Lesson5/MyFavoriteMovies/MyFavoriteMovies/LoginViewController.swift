...
class LoginViewController: UIViewController {
    ...
    private func getRequestToken() {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            guard let statusCode = (response as? HTTPURLResponse)?.statusCode, statusCode >= 200 && statusCode <= 299 else {
                displayError("Request returned with a status code other than 2xx!")
                return
            }
            
            guard let data = data else {
                displayError("No data was return by request!")
                return
            }
            ...
            if let _ = parsedResult[Constants.TMDBResponseKeys.StatusCode] as? Int {
                displayError("TheMovieDB return an error. See '\(Constants.TMDBResponseKeys.StatusCode)' and '\(Constants.TMDBResponseKeys.StatusMessage)' in \(parsedResult)")
                return
            }
            
            guard let requestToken = parsedResult[Constants.TMDBResponseKeys.RequestToken] as? String else {
                displayError("Could not find key '\(Constants.TMDBResponseKeys.RequestToken)' in \(parsedResult)")
                return
            }
    ...
    private func loginWithToken(_ requestToken: String) {
        ...
        let methodParameters = [Constants.TMDBParameterKeys.ApiKey: Constants.TMDBParameterValues.ApiKey,
                                ...
                                Constants.TMDBParameterKeys.RequestToken: requestToken] as [String : Any]
        /* 2/3. Build the URL, Configure the request */
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            // status code
            guard let statusCode = (response as? HTTPURLResponse)?.statusCode, statusCode >= 200 && statusCode <= 299 else {
                displayError("Response returned with stauts code other than 2xxx!")
                return
            }
            // data
            guard let data = data else {
                displayError("Response returned no data")
                return
            }
            ...
            guard let success = parsedResult[Constants.TMDBResponseKeys.Success] as? Bool, success == true else {
                displayError("Could not find key '\(Constants.TMDBResponseKeys.Success))' in \(parsedResult)")
                return
            }
    ...
    private func getSessionID(_ requestToken: String) {
        ...
        /* 2/3. Build the URL, Configure the request */
        let request = URLRequest(url: self.appDelegate.tmdbURLFromParameters(methodParameters as [String:AnyObject], withPathExtension: "/authentication/session/new"))

        /* 4. Make the request */
        let task = self.appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            // if an error occurs, print it and re-enable the UI
            func displayError(_ error: String) {
                print(error)
                performUIUpdatesOnMain {
            ...
            // error
            guard (error == nil) else {
                displayError("Response returned an error: \(String(describing: error))")
                return
            }
            // status code
            guard let statusCode = (response as? HTTPURLResponse)?.statusCode, statusCode >= 200 && statusCode <= 299 else {
                displayError("Response return status code other than 2xx!")
                return
            }
            // data
            guard let data = data else {
                displayError("Response return no data!")
                return
            }
            ...
            if let _ = parsedResult[Constants.TMDBResponseKeys.StatusCode] as? Int {
                displayError("TheMovideDB return an error. Check '\(Constants.TMDBResponseKeys.StatusCode)' and '\(Constants.TMDBResponseKeys.StatusMessage)' in \(parsedResult)")
                return
            }
            ...
            guard let sessionID = parsedResult[Constants.TMDBResponseKeys.SessionID] as? String else {
                displayError("Could not find key '\(Constants.TMDBResponseKeys.SessionID)'  in \(parsedResult)")
                return
            }
    ...
    private func getUserID(_ sessionID: String) {
        ...
        let task = self.appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            // error
            guard (error == nil) else {
                displayError("Request return an error: \(String(describing: error))")
                return
            }
            ...
            // data
            guard let data = data else {
                displayError("Request returned no data!")
                return
            }
            ...
            if let _ = parsedResult[Constants.TMDBResponseKeys.StatusCode] as? Int {
                displayError("TheMovieDB returned an error. Check '\(Constants.TMDBResponseKeys.StatusCode)' in '\(Constants.TMDBResponseKeys.StatusMessage)' in \(parsedResult)")
                return
            }
            
            guard let userID = parsedResult[Constants.TMDBResponseKeys.UserID] as? Int else {
                displayError("Could not find key '\(Constants.TMDBResponseKeys.UserID)' in \(parsedResult)")
                return
            }
